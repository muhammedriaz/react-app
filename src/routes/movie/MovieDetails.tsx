        ...
        <>
            <div className="flex p-5 w-full relative">
                ...
                <div className="w-1/3 relative">
                    <img className="w-100 h-100" src={`https://media.themoviedb.org/t/p/w600_and_h900_bestv2/${movie.poster_path}`} alt={movie.title}/>
                </div>
                <div className="flex flex-col gap-6 w-2/3 pl-5 text-white relative">
                    <div>
                        <h1 className="text-3xl font-bold">{movie.title} ({moment(movie.release_date).format('YYYY').toString()})</h1>
                        <h4 className="text-lg text-gray-300">{moment(movie.release_date).format('LL').toString()} | {getMovieTime(movie.runtime)}</h4>
